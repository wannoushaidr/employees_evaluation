                    ...
                    if opt.track:
                        ...
                        if len(tracked_dets)>0:
                            ...
                            for face in faces:  
                                ...
                                # Draw face bounding boxes (optional)  
                                cv2.rectangle(im0, (x, y), (x + w, y + h), (255, 0, 0), 2)  
                                # Draw landmarks  
                                shape = shape_predictor(im0, face)  # Get the landmarks  
                            ...
                            for face_id, time_info in face_time_spent.items():  
                                ...
                                cv2.putText(im0, f"Time: {duration:.2f}s", (x_last, y_last), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)  

                                # Display the time spent above the face  
                               # cv2.putText(im0, f"Time: {duration:.2f}s", (x+40, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)  
                            # Cleanup: Remove any faces that have disappeared (not detected in current frame)  
                            detected_face_ids = {identity for identity in identities}  # Update to currently detected  
