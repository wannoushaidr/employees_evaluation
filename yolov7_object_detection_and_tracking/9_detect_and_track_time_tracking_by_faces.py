...
def draw_boxes(img, bbox, identities=None, categories=None, confidences = None, names=None, colors = None):
    ...
    for i, box in enumerate(bbox):
        ...
        if not opt.nobbox: 
         ...
         # Draw space above the bounding box  
        ...
        area_text = f'Area: {area_meters} m'  
        tf = max(tl - 1, 1)  # font thickness  
        t_size = cv2.getTextSize(area_text, 0, fontScale=tl / 3, thickness=tf)[0]  
        c2 = x1 + t_size[0], y1 - label_height - t_size[1] - 3  # Position for text  
                    ...
                    if opt.track:
                        ...
                        if len(tracked_dets)>0:
                            ...
                            for face in faces:  
                                ...
                                for idx, identity in enumerate(identities):  
                                    ...
                                    if (x >= bbox[0] and x + w <= bbox[2]) and (y >= bbox[1] and y + h <= bbox[3]):  
                                        # If face found for an object, save its ID and start timer  
                                        face_id = identity  
                                            
                                        if face_id not in face_time_spent:  
                                            face_time_spent[face_id] = {'start_time': current_time, 'duration': 0,'active': True}  
                                            
                                            face_id_map[face_id] = face  # Map face_id to its rectangle  
                                        face_time_spent[face_id]['active'] = True

                                        break  # Break after first match  
                
                                # If face_id is still None, it means it's a new detected face  
                                if face_id is None:  
                                ...
                                # Update bounding box and visuals  
                                cv2.rectangle(im0, (x, y), (x + w, y + h), (255, 0, 0), 2)  
                                shape = shape_predictor(im0, face)  # Get landmarks  
                                for i in range(68):  # 68 landmarks  
                                    landmark_x = shape.part(i).x  
                                    landmark_y = shape.part(i).y  
                                    cv2.circle(im0, (landmark_x, landmark_y), 2, (0, 255, 0), -1)  # Draw landmarks  
                    
                            # Update durations for active faces  
                            for face_id, time_info in face_time_spent.items():  
                                if 'active' in time_info and time_info['active']:  # Check if 'active' key exists  
                                    duration = current_time - time_info['start_time']  
                                    face_time_spent[face_id]['duration'] = duration  
                                    face_time_spent[face_id]['active']=False
                                    # Draw the time spent above the face  
                                    if face_id in face_id_map:  # Check if the face_id is in the map  
                                        last_face = face_id_map[face_id]  # Get the rectangle associated with the face_id  
                                        x_last, y_last = last_face.left(), last_face.top() - 10  # Place text above the bounding box  
                                        cv2.putText(im0, f"Time: {int(duration)}s", (x_last, y_last), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)  

                                    
                            # Cleanup: Remove faces that have disappeared  
                            ...
                            #for face_id in list(face_time_spent.keys()):  # Use list to avoid modifying during iteration  
                                    ...
                                    #if face_id in face_id_map:  # Check if it's in the map before deleting
                                        #del face_id_map[face_id]  # Optionally remove from the mapping  
