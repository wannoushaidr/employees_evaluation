...
def draw_boxes(img, bbox, identities=None, categories=None, confidences=None, colors=None, time_spent=None, opt=None, names=None):  
...
#**************************************************
    ...
    while True:
        ...
        for path, img, im0s, vid_cap in dataset:
            ...
            if dataset.mode == 'video' and frame_counter % 20 != 0:  
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if len(det):  
                    ...
                    #categories = det[:, 4].astype(int)  # Assuming the sixth column is the category index  
                    #identities = det[:, 4]  # Assuming the fifth column is the identity
...
#****************************************************************************************************************************************************
                        ...
                        img, centers = draw_boxes(im0, bbox_xyxy, identities, categories, confidences=None, colors=colors, time_spent=time_spent, opt=opt, names=names)  
...
#put employee for customer                           
                        ...
                        for i in range(len(keys)):  
                            for j in range(i + 1, len(keys)):  # Ensure pairs are not repeated  
                                ...
                                if(distance<1.4 and  keys[i] and keys[j] not in not_available_people ):
                                    print("ss")
                                    not_available_people.append(keys[i])
                                    not_available_people.append(keys[j])
                                    saved_cople[cople_id]={keys[i]:face_time_spent[keys[i]]['start_time'],keys[j]:face_time_spent[keys[j]]['start_time']}
                                    cople_id=+1
                                    cople_number=+1
                                    print('cople_number = ',cople_number)
